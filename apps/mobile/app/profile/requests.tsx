...
export default function Requests() {
  ...
  return <Screen><Title subtitle="Send one YouTube video or Spotify track link per request.">Request music</Title><Input placeholder="YouTube or open.spotify.com/track/..." autoCapitalize="none" autoCorrect={false} value={text} onChangeText={setText} /><Button title="Send request" loading={busy} onPress={() => void submit()} /><Text style={ui.section}>History</Text>{items.length ? items.map(item => <Card key={item.id}><View style={[ui.row, { justifyContent: "space-between", alignItems: "flex-start" }]}><View style={{ flex: 1 }}><Pressable onPress={() => item.sourceUrl && void Linking.openURL(item.sourceUrl)}><Text style={ui.body}>{item.requestedTitle ? `${item.requestedTitle}${item.requestedArtist ? ` — ${item.requestedArtist}` : ""}` : item.songsRequested}</Text></Pressable><Text style={[ui.muted, { marginTop: 8, color: item.status === "Accepted" ? colors.accent : item.status === "Rejected" ? colors.red : colors.muted }]}>{item.status} • {new Date(item.requestDate!).toLocaleString()}</Text>{item.rejectionReason ? <Text style={[ui.body, { color: colors.red, marginTop: 8 }]}>Reason: {item.rejectionReason}</Text> : null}</View><Pressable onPress={() => void remove(item.id)}><Text style={{ color: colors.muted }}>Delete</Text></Pressable></View></Card>) : <Empty label="No music requests yet" />}</Screen>;
}

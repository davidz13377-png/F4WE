...
export default function Staff() {
  ...
  return <Screen><Title subtitle="Review user submissions and send status notifications.">Staff Portal</Title><Button title="Music Manager" icon="trash-outline" tone="dark" onPress={() => router.push("/profile/music-manager")} /><View style={{ height: 16 }} /><View style={[ui.row, { gap: 8, marginBottom: 18 }]}><View style={{ flex: 1 }}><Button title="Music requests" tone={tab === "requests" ? "green" : "dark"} onPress={() => setTab("requests")} /></View><View style={{ flex: 1 }}><Button title="Bug reports" tone={tab === "bugs" ? "green" : "dark"} onPress={() => setTab("bugs")} /></View></View>
    {list.length ? list.map(item => <Card key={item.id}><View style={[ui.row, { justifyContent: "space-between" }]}><Text style={[ui.body, { fontWeight: "800" }]}>{item.user?.username}</Text>{item.user ? <RankBadge rank={item.user.rank} /> : null}</View><Text style={[ui.body, { marginTop: 12 }]}>{tab === "requests" ? item.songsRequested : item.description}</Text><Text style={[ui.muted, { marginTop: 8 }]}>Status: {item.status}</Text>{tab === "requests" && item.sourceUrl ? <View style={[ui.row, { gap: 16, marginTop: 12 }]}><Pressable accessibilityLabel="Open YouTube video" onPress={() => void Linking.openURL(item.sourceUrl!)}><Ionicons name="logo-youtube" size={30} color="#FF3030" /></Pressable>{item.status === "Pending" ? <Pressable accessibilityLabel="Import song" onPress={() => { setImportId(item.id); setTitle(""); setArtist(""); }}><Ionicons name="add-circle-outline" size={32} color={colors.accent} /></Pressable> : null}</View> : null}{item.rejectionReason ? <Text style={{ color: colors.red, marginTop: 7 }}>{item.rejectionReason}</Text> : null}{item.status === "Pending" ? <View style={[ui.row, { gap: 8, marginTop: 14 }]}>{tab === "bugs" || !item.sourceUrl ? <View style={{ flex: 1 }}><Button title={tab === "requests" ? "Accept" : "Fixed"} onPress={() => void review(tab, item.id, tab === "requests" ? "Accepted" : "Fixed")} /></View> : null}<View style={{ flex: 1 }}><Button title="Reject" tone="red" onPress={() => setReject({ kind: tab, id: item.id })} /></View></View> : null}</Card>) : <Empty label="Nothing to review" />}

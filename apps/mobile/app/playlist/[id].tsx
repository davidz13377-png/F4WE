...
export default function PlaylistScreen() {
  ...
  return <Screen><Stack.Screen options={{ title: "Playlist" }} />
    ...
    {playlist ? <><View style={{ alignItems: "center", marginBottom: 18 }}><Pressable disabled={!owned || busy} onPress={() => void changePicture()} accessibilityRole="button" accessibilityLabel={owned ? "Change playlist picture" : "Playlist picture"}>{playlist.artworkUrl ? <Image source={{ uri: profilePictureUrl(playlist.artworkUrl) }} style={{ width: 190, height: 190, borderRadius: 18 }} /> : <View style={{ width: 190, height: 190, borderRadius: 18, backgroundColor: colors.raised, alignItems: "center", justifyContent: "center" }}><Ionicons name="musical-notes" size={68} color={colors.accent} /></View>}{owned ? <View style={{ position: "absolute", right: 8, bottom: 8, backgroundColor: colors.accent, width: 38, height: 38, borderRadius: 19, alignItems: "center", justifyContent: "center" }}>{busy ? <ActivityIndicator size="small" color={colors.accentText} /> : <Ionicons name="camera" size={19} color={colors.accentText} />}</View> : null}</Pressable>{owned && playlist.artworkUrl ? <Pressable onPress={removePicture} style={{ padding: 10 }}><Text style={{ color: colors.red }}>Remove picture</Text></Pressable> : null}</View><Title subtitle={"Playlist • " + playlist.creator.username + " • " + playlist.songs.length + " songs"}>{playlist.name}</Title>
      {playlist.description ? <Text style={[ui.body, { marginBottom: 16 }]}>{playlist.description}</Text> : null}
      {owned ? <View style={[ui.row, { justifyContent: "space-between", marginBottom: 16 }]}><Text style={ui.body}>{playlist.isPublic ? "Public: visible in Search" : "Private: only you can open it"}</Text><Switch value={playlist.isPublic} disabled={busy} onValueChange={value => void changeVisibility(value)} /></View> : null}
      {playlist.songs.length ? <Button title="Play playlist" icon="play" onPress={() => void player.play(playlist.songs[0], playlist.songs)} /> : null}
      ...
      {playlist.songs.map(song => <SongRow key={song.id} song={song} queue={playlist.songs} onRemove={owned ? () => remove(song.id) : undefined} />)}

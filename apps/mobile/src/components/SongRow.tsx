...
export function SongRow({ song, queue, onPlay, onRemove }: { song: Song; queue?: Song[]; onPlay?: () => void; onRemove?: () => void }) {
  const player = usePlayer();
  const library = useLibrary(); const liked = library.isFavorite(song);
  const play = () => { onPlay?.(); void player.play(song, queue); };
  return <View style={styles.row}>
    ...
    <View style={{ flex: 1 }}><Text numberOfLines={1} style={styles.title}>{song.title}</Text><Text numberOfLines={1} style={styles.artist}>{song.artist || "Unknown artist"}</Text></View>
    </Pressable>
...
const styles = StyleSheet.create({ row: { flexDirection: "row", alignItems: "center", paddingVertical: 9 }, action: { width: 40, height: 44, justifyContent: "center", alignItems: "center" }, art: { width: 46, height: 46, borderRadius: 10 }, fallback: { backgroundColor: colors.raised, alignItems: "center", justifyContent: "center" }, title: { color: colors.text, fontWeight: "700", fontSize: 15 }, artist: { color: colors.muted, fontSize: 13, marginTop: 3 } });
